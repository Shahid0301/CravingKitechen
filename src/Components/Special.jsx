function Special({ index, data }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center flex-col w-1/2 space-y-5 hover:text-white p-3 sm:space-y-1">
        <h1 className="font-playfair text-2xl font-bold text-gray-700 hover:text-white sm:text-xs">
          {data.title}
        </h1>
        <span className="font-playfair text-center p-1 sm:text-xs">
          {data.description}
        </span>
        {/* Child */}
        <h1 className="text-4xl text-yellow-100 group-hover:text-white sm:text-base">
          &#8377;{data.price}/-
        </h1>
      </div>
    </div>
